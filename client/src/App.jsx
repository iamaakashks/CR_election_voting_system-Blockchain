...
import { ShieldCheck, Vote, Users, Landmark, Loader2, AlertTriangle } from 'lucide-react';
...
const VotingDashboard = ({ userInfo, onLogout }) => {
    ...
    const handleVote = async (electionId, candidateId) => {
        try {
            ...
            // The API call to cast the vote
            await axios.post(`http://localhost:5001/api/elections/${electionId}/vote`, body, config);

            // If successful, update the UI to show the 'Thank You' screen
            setVotedStates(prev => ({ ...prev, [electionId]: true }));
        } catch (err) {
            // If the API returns an error (e.g., "already voted"), show it
        ...
        <div className="bg-gray-50 min-h-screen text-gray-800">
             <header className="bg-white shadow-sm border-b">
                 <div className="container mx-auto px-6 py-4 flex flex-col sm:flex-row justify-between items-center">
                     <div className="flex items-center mb-2 sm:mb-0"><Landmark className="text-indigo-600 mr-3 h-8 w-8" /><h1 className="text-2xl font-bold text-indigo-600">College Voting Portal</h1></div>
                    <div className="text-center sm:text-right"><p className="font-semibold">Welcome, {userInfo.name}</p><button onClick={onLogout} className="text-sm text-indigo-600 hover:underline">Logout</button></div>
                </div>
            </header>
                            ...
                            elections.map((election) => (
                                <div key={election._id}>
                                    {votedStates[election._id] ? (
                                        ...
                                        <Card>
                                            <div className="flex justify-between items-start"><h4 className="text-2xl font-bold text-indigo-700">{election.title}</h4><span className="bg-green-100 text-green-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">{election.status}</span></div>
                                            <hr className="my-6" />
...
function App() {
    ...
    return (<> {userInfo ? <VotingDashboard userInfo={userInfo} onLogout={logoutHandler} /> : <LoginScreen onLoginSuccess={loginSuccessHandler} />} </>);
}
